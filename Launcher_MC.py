...
def launch_greyboxmodel(**kwargs):
    ...
    if "f_physical_model" in kwargs:
        ...
        f_userdefined = user_config_mc.user_params["f_physical_model"]
    ...
    if "h_physical_model" in kwargs:
        ...
        h_userdefined = user_config_mc.user_params["h_physical_model"]

    if "n_x" in kwargs:
        n_x = kwargs["n_x"]
    else:
        n_x = user_config_mc.user_params["n_x"]

    if "n_y" in kwargs:
        n_y = kwargs["n_y"]
    else:
        n_y = user_config_mc.user_params["n_y"]

    if "n_u" in kwargs:
        n_u = kwargs["n_u"]
    else:
        n_u = user_config_mc.user_params["n_u"]

    if "n_x_b" in kwargs:
        n_x_b = kwargs["n_x_b"]
    else:
        n_x_b = user_config_mc.user_params["n_x_b"]

    if "theta_f_initial_guess" in kwargs:
        theta_f_initial_guess = kwargs["theta_f_initial_guess"]
    else:
        theta_f_initial_guess = user_config_mc.user_params["theta_f_initial_guess"]

    if "theta_h_initial_guess" in kwargs:
        theta_h_initial_guess = kwargs["theta_h_initial_guess"]
    else:
        theta_h_initial_guess = user_config_mc.user_params["theta_h_initial_guess"]

    if "x0_phys_initial_guess" in kwargs:
        x0_phys_initial_guess = kwargs["x0_phys_initial_guess"]
    else:
        x0_phys_initial_guess = user_config_mc.user_params["x0_phys_initial_guess"]

    if "x_b_0" in kwargs:
        x_b_0 = kwargs["x_b_0"]
    else:
        x_b_0 = user_config_mc.user_params["x_b_0"]
    ...
    if "min_cons_hard_x_step" in kwargs:
        ...
        min_cons_hard_x_step = user_config_mc.user_params["min_cons_hard_x_step"]

    if "max_cons_hard_x_step" in kwargs:
        max_cons_hard_x_step = kwargs["max_cons_hard_x_step"]
    else:
        max_cons_hard_x_step = user_config_mc.user_params["max_cons_hard_x_step"]

    if "min_cons_hard_y_step" in kwargs:
        min_cons_hard_y_step = kwargs["min_cons_hard_y_step"]
    else:
        min_cons_hard_y_step = user_config_mc.user_params["min_cons_hard_y_step"]

    if "max_cons_hard_y_step" in kwargs:
        max_cons_hard_y_step = kwargs["max_cons_hard_y_step"]
    else:
        max_cons_hard_y_step = user_config_mc.user_params["max_cons_hard_y_step"]
    ...
    # Receiving Monte Carlos Dataset Input/Output Data for Cascaded Tanks

    if "sample_idx" in kwargs:
        sample_idx = kwargs["sample_idx"]
    else:
        print("No sample idx was provided. Setting sample_idx = 0")
        sample_idx = 0

    with open('dicts_mc/multidataset_gen_dict.pkl', 'rb') as f:
        multidataset_gen_dict = pk.load(f)

    u_train = torch.tensor(multidataset_gen_dict[sample_idx][0]["u_train"], dtype=torch.float32)
    y_train = torch.tensor(multidataset_gen_dict[sample_idx][0]["y_train"], dtype=torch.float32)
    u_val  = torch.tensor(multidataset_gen_dict[sample_idx][0]["u_test"], dtype=torch.float32)
    y_val  = torch.tensor(multidataset_gen_dict[sample_idx][0]["y_test"], dtype=torch.float32)
    u_test  = torch.tensor(multidataset_gen_dict[sample_idx][0]["u_test"], dtype=torch.float32)[6:]
    y_test  = torch.tensor(multidataset_gen_dict[sample_idx][0]["y_test"], dtype=torch.float32)[6:]
